...
int BikeSystem::search(const char *user_name, int start_x, 
    int start_y, int acceptive_comfort_rating) const {
//请在此输入实现，请不要更改其余已有api
    int ans = -1;
    for(int i=0; i<size; i++){
        if(!bikes[i]->get_occupied() && bikes[i]->get_comfort()>=acceptive_comfort_rating){
            if(ans==-1) ans = i;
            else{
                if(bikes[i]->compute_dist(start_x, start_y) 
                    < 
                    bikes[ans]->compute_dist(start_x, start_y)){
                    ans = i;
                }
                else if(bikes[i]->compute_dist(start_x, start_y) 
                    == 
                    bikes[ans]->compute_dist(start_x, start_y)){
                    ans = (bikes[i]->get_comfort()>bikes[ans]->get_comfort())?i:ans;
                }
            }
        }
    }
    if(ans==-1)
        ...
        cout << user_name << " selected " << bikes[ans]->get_serial();
    cout <<endl;
	return ans;
}
